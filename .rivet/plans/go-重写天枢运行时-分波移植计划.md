...
> **Status: APPROVED** — 2026-09-18T16:15:55.756Z
...
1. **JSON 字节稳定性 —— 已实测判决（2026-09-19，Go/No-Go 门通过）**。用真实 oracle `src/api/stable-json.ts` 对账 Go `encoding/json`（`SetEscapeHTML(false)`）24 组用例，结论如下：

   | 维度 | Node (oracle) | Go stdlib | 判定 |
   |------|---------------|-----------|------|
   | `&<>` 转义 | `{"x":"<a>&b</a>"}` | 同 | ✅ 原假设已由 `SetEscapeHTML(false)` 解决 |
   | ASCII 键序 | `{"C":2,"_z":3,"a":1,"b":"x"}` | 同 | ✅ |
   | **非 ASCII 键序** | `Z _ a é ÿ 中 𐀀 🎉 \ue000 \ufffd` | `Z _ a é ÿ 中 \ue000 \ufffd 𐀀 🎉` | ❌ **位置 3–9 全错位** |
   | 负零 | `0` | `-0` | ❌ |
   | 大整数 | `9007199254740992`（丢精度到 2^53） | `9007199254740993`（精确） | ❌ |
   | U+2028/29 | 原样输出 | `\u2028\u2029` | ❌ |
   | 数组内 undefined | `[1,,2]`（**非法 JSON**） | — | ⚠️ oracle 自身缺陷 |

   **真正的威胁不是原计划假设的 `&<>` 转义（已解决），而是键排序**：Go `sort.Strings` 按 UTF-8 字节序，JS `Object.keys().sort()` 按 UTF-16 code unit 序——BMP 外字符（代理对 `D800–DFFF`）与 U+E000–U+FFFF 区间的相对顺序完全相反。这是会**静默击穿缓存**的差异。

   对策：`internal/api/stablejson` 不能直接用 `encoding/json`，需自实现适配层——① 键比较按 UTF-16 code unit 序（非 UTF-8 字节序）；② 负零归一为 `0`；③ 数字按 JS float64 语义格式化（`1e+21` / `0.000001` / `1e-7` 三种形态已实测与 Go 一致，仅需处理 `-0` 与大整数）；④ U+2028/29 不转义。⑤ **不复制 oracle 的数组 undefined 缺陷**（产出非法 JSON）——Go 侧应显式报错或跳过。

   **Go/No-Go 判决：字节等价可达**（ASCII 载荷已实测一致，真实请求体键全为 ASCII），**无需降级为「缓存 key 等价」**。但适配层是必需的，不是可选的。
...
4. **Wave 0 的夹具必须来自真实运行而非手工构造**——手工构造的 fixture 会编码「我以为的」输入分布，掩盖真实路径。导出脚本从实际 `AgentLoop` 运行中捕获。
   ...
   **⚠️ 本条已被违反一次，事故记录（2026-09-19）**：wire oracle 的字段序是**手抄**的，写成 `messages→model→stream`，而真实顺序是 `model→messages→stream`（`openai-client.ts:474-476`）。Go 实现与 golden **双方同错**，测试照绿——报告的「795 字节逐字节一致」是与自造 golden 自洽的**假绿**。修复：oracle 生成器改为 mock `globalThis.fetch` 捕获**真实 `OpenAIClient.stream()` 实际发送的字节**（`captureRawBody`），并加顶层键序断言作第二判据；反证确认把 Go 实现改回错误顺序即报「首差 @3」+ 键序不符。

   **推广纪律**：oracle 生成器必须调用被测的**真实代码路径**；任何手抄的常量（字段序、默认值、枚举）都会引入自洽假绿。此外，**假绿不会被自身的绿灯暴露**——必须用变异反证（改回错误实现看是否变红）才能证伪。

**本计划自身可被推翻的方式（已于 2026-09-19 执行并得出结论）**：计划原定「若字节对账发现无法弥合的差异，则降级为缓存 key 等价」。该探针已在开工前跑完，结论是**差异存在但可弥合**——`&<>` 转义已解决，非 ASCII 键序需适配层（见「三个高危差异点」第 1 条的实测表）。**目标维持「字节等价」，不降级。**

**残留不确定性的最终裁决（2026-09-19，真实端点实测）**：计划原列「真实请求体是否含非 ASCII 键」为唯一残留不确定性，由 Wave 1 的真实端点冒烟裁决。该冒烟已执行——指向 `ai.ctaigw.cn/v1` + `deepseek-v4.1-flash` 多轮实测，前缀缓存稳定命中 **93.7%–95.5%**（轮1 1608→1536、轮2 1617→1536、轮3 1630→1536、轮4 1639→1536）。**字节等价成立，缓存 key 与 TS 版一致**——Go/No-Go 门通过。真实模型下的完整闭环亦已跑通（模型自主读 → 改 → 跑测试验证，命中 85.6%）。

## 风险与依赖

| 风险 | 影响 | 缓解 |
|------|------|------|
| JSON 字节差异击穿前缀缓存 | 成本翻倍，缓存命中率归零 | **已实测定位**：`&<>` 已解决，非 ASCII 键序需 UTF-16 适配层（见高危差异点 1）。Wave 1 前置实现 + 真实端点冒烟验证 |
