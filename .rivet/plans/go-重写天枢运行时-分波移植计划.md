...
**本计划自身可被推翻的方式（已于 2026-09-19 执行并得出结论）**：计划原定「若字节对账发现无法弥合的差异，则降级为缓存 key 等价」。该探针已在开工前跑完，结论是**差异存在但可弥合**——`&<>` 转义已解决，非 ASCII 键序需适配层（见「三个高危差异点」第 1 条的实测表）。**目标维持「字节等价」，不降级。** 唯一残留的不确定性是真实请求体是否含非 ASCII 键（若含，则适配层从「正确性保险」升级为「必需项」）——这一条由 Wave 1 的真实端点冒烟（`cache_read_input_tokens > 0`）裁决。
