...
>
> 姊妹文档：`go/HANDOFF.md`（**5638 行，单一事实来源**——每刀一个专章，含欠账/踩坑/验证记录）。
...
> **进度权威是 `go/HANDOFF.md`**（截至 2026-09-23 为**第四十八刀**）。
> 下表是本文的实测快照，**会滞后**——引用前用下方复现命令自测。
...
# 已注册工具数（对照 TS full preset 的 51 目标）
grep -cE '^\s+(r|out)\.Register\(' internal/tools/default_registry.go
```

**关于 TS 行数对照**：`src/` 下非测试 `.ts` 实测 **63,525 行**
...
**工具基数是 51**（不是 44，也不是 21）：TS 侧 `tool-preset.ts:16` 与
`tool-preset.test.ts:100` 的 `totalCount('full')` 断言均为 **51**
（minimal 30 / frontend 31 / full 51；桌面端有调度器时 full +3 = 54）。
`default_registry.go:20` 注释写的「完整 44 个工具是分波目标」**已过期**——
44 是 TS 3.x 早期快照，现为 51。旧版本文档的「10 / 21」更早已过时。

**必须建立的认知**：这不是"快完成了"，而是**地基已夯实、主体未动**。已完成的深度
（字节对账、变异反证、oracle 纪律）很扎实，但广度上 TS 的多个核心目录
（`auth`/`model`/`mcp`/`lsp`/`repo`/`search`/`tui`/`server`/`config`/`workers`/`plugins`/`memory`）
在 `internal/` 下**尚无对应目录**（实测 12 个全缺）。
...
> Wave 3/4 线，止于第二十八刀）。`go/HANDOFF.md` 另有**回主线编号**
> （从第一刀重新计数，**截至 2026-09-23 为第四十八刀**，覆盖 trust/skill/plan
> 工具链与 hook 分派）。
> **两套编号互不连续**——引用时务必说明是哪一套。
>
> **进度以 `go/HANDOFF.md` 为准**（它是单一事实来源）。本节保留旧编号是因为
> 各刀的**设计意图与依赖关系**仍有参考价值。
>
> **⚠️ 本节以下的「下一刀」清单已滞后约 20 刀**——实测（2026-09-24）：
> 其中 `summarize.go`、`computeModelReadCap`/`getFileReadMtime` 已落地；
> `TaskAnchor` 半落地（`checkpoint.go:49` 有字段与调用点，但 `main.go` 未注入、
> Go 侧无 `getActiveContract`/`renderTaskAnchor`，**仍是悬空字段**）；
> `resolveCompactionEconomics`、`Advisor.onTurnEnd`、LLM 重写路径**未落地**。
> **接手时先读 `go/HANDOFF.md` 的「下一步」章节**（那里的建议时效性更新，
> 且明确记载「连续十一刀的估价全部被证伪或偏轻」——建议清单天然短时效）。
...
- **本文 §4 的「下一刀」编号也已滞后**——真实进度见 `go/HANDOFF.md` 的最新专章
  （**截至 2026-09-23 为第四十八刀**：补 verify 声明的信任门）。
- **本文 §1 的指标表、§8 的期望包数、§9 的资产清单同样滞后**——已按 2026-09-24
  实测回填，但下次接手前仍应自测（§1 有复现命令）。

**建议**：把 `go/HANDOFF.md` 当作**单一事实来源**，本文只保留**环境重建**与
...
- [ ] 期望 `22 包 ok / 0 FAIL`（2026-09-24 实测；`go list ./...` 报 26 个包，含无测试的）
- [ ] 读 §3 开发纪律（**尤其 §3.2 变异反证的红 0 成因清单**）
- [ ] 读 `go/HANDOFF.md` 的「下一步」确定第一刀（**不要按本文 §4 的旧清单开工**）
...
1. **oracle 纪律**——61 个数据集（2026-09-24 实测，写本节时为 31 个），
   全部从真实 TS 路径导出，全部可复现
2. **变异反证习惯**——每个改动都有"改错看是否变红"的验证
3. **踩坑知识**——`go/HANDOFF.md` 记录了编译失败伪装、假绿事故、等价变异判据等

**当前最大的风险**：**广度不足**。地基（API 层、工具内核、prompt 静态层）很扎实，
但表面层与集成层整体未启动——实测 `internal/` 下缺 12 个 TS 核心目录
（`auth`/`model`/`mcp`/`lsp`/`repo`/`search`/`tui`/`server`/`config`/`workers`/`plugins`/`memory`）。
若只在地基上继续加深，会**偏离"可运行的 agent 运行时"这个目标**。

> **2026-09-24 修正**：本节原写「hook 管线、认知层、压缩、缓存全部未动」——
> **该判断已过期**。截至第四十八刀，hook 管线（五阶段 + 多个 hook 模块）、
> 认知层（claims / advisory bus / readback / 习惯化）、压缩（策略层 + 执行层 +
> session split 闭环）、缓存（`internal/cache` advisor + warmth）**均已落地并接线**。
> 真正的缺口已转移到**表面层与集成层**（TUI / server / MCP / LSP / auth / model）。

**建议的节奏**（原文保留，但前提已变）：先用 2-3 轮把 hook 管线立起来——
**此项已完成**；当前建议改为按 `go/HANDOFF.md` 的「下一步」章节推进
（那里记载：动手前必须 grep 核实消费端，连续十一刀的估价全部被证伪或偏轻）。
